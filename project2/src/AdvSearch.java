...
public class AdvSearch extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String query = "SELECT DISTINCT M.id, M.title, M.year, M.director "
							+ "FROM movies M, stars S, stars_in_movies SIM WHERE";

			query += appendAndToQuery(title, year, director, name);
	...
	public String appendAndToQuery(String myTitle, String myYear, String myDir, String myName) {
		String query = "";
		if (myTitle != "") {
			String titleConstraint = " title = \"" + myTitle + "\"";
			query += titleConstraint;
		}
		if (myYear != "") {
			String yearConstraint = " year = \"" + myYear + "\"";
			if (myTitle != "")
				query += " AND";
			query += yearConstraint;
		}
		if (myDir != "") {
			String directorConstraint = " director = \"" + myDir + "\"";
			if (myTitle != "" || myYear != "")
				query += " AND";
			query += directorConstraint;
		}
		if (myName != "") {
			String nameConstraint = " S.name = \"" + myName + "\"";
	...
	public String appendOrderBy(String sortTitle, String sortYear) {
		String query = "";
		if (sortTitle.equals("true")) {
			query += " ORDER BY M.title"; 
		}
		if (sortYear.equals("true")) {
